...
while [ true ]; do
  ...
  read a1
  StatusBarApp_POPUP="$(/usr/local/sbin/io_wnu_popup -title 'Wi-Fi Disabled' -subtitle "$CHECK_SERVICE" -message 'Actions?' -actions 'Enable Wi-Fi',"Switch Service","Show/Hide Utility" -timeout 30 -appIcon "$APP"/Contents/Resources/ModelIcon.icns)"
  #-sound default)
  ...
  result=connected
  StatusBarApp_POPUP="$(/usr/local/sbin/io_wnu_popup -title 'Wi-Fi Enabled' -subtitle "$CHECK_SERVICE" -message 'All is well?' -actions "Disable Wi-Fi","Switch Service","Show/Hide Utility" -timeout 15 -appIcon "$APP"/Contents/Resources/ModelIcon.icns)"
  #-sound default)
      case $StatusBarApp_POPUP in
        "@TIMEOUT") echo "OK" ;;
        "@CLOSED") echo "You clicked on the default alert' close button" ;;
